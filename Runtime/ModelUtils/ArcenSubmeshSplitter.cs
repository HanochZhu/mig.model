...
namespace Mig.Model.Utils
{
    public class ArcenSubmeshSplitter
    {
        ...
        public static void ProcessGameObject(MeshFilter meshFilterComponent)
        {
            if (!meshFilterComponent)
            {
                Debug.LogError("MeshFilter null for '" + meshFilterComponent.name + "'!");
                return;
            }
            MeshRenderer meshRendererComponent = meshFilterComponent.GetComponent<MeshRenderer>();
            if (!meshRendererComponent)
            {
                Debug.LogError("MeshRenderer null for '" + meshFilterComponent.name + "'!");
                return;
            }
            Mesh mesh = meshFilterComponent.GetComponent<MeshFilter>().sharedMesh;
            if (!mesh)
            {
                Debug.LogError("Mesh null for '" + meshFilterComponent.name + "'!");
                return;
            }
            ...
            {
                Debug.LogError("List<MeshFromSubmesh> empty or null for '" + meshFilterComponent.name + "'!");
                return;
            }
            string goName = meshFilterComponent.name;
        ...
        public static void ProcessGameObject(GameObject go)
        {
            // Isolate Sub Meshes
            MeshFilter meshFilterComponent = go.GetComponent<MeshFilter>();
            ProcessGameObject(meshFilterComponent);
        }
    ...
    public static class MeshExtension
    {
        private class Vertices
        {
            ...
            private void Copy<T>(ref List<T> aDest, List<T> aSource, int aIndex)
            {
                ...
                {
                    Debug.Log("[Mig] Error , the mesh has an error indice in verts arrya");
                    return;
                }
